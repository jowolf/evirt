...
class VirtualMachineBase:
    ...
    def __init__ (self, **kw):
        initial_dict = dict (
            ...
            up      = kw.pop ("up", False),
            ports   = kw.pop ("ports", []), # list of open ports - for live vm only! (up==True)
            pid     = kw.pop ("pid", 0),    # program id
            ...
            wd      = kw.pop ("wd", 0),     # watch descriptor, for iNotify changes to parms or status - only valid on server
...
class LocalVM (VirtualMachineBase):
    def __init__ (self, **kw):      # possibly pass in path, & do screengrab, etc here? replace vmImagePath?
...
class RemoteVM (VirtualMachineBase):
    ...
    def monitor_command (self, cmd):  # kvm monitor command (or list)
        ...
        if isinstance (cmd, (list, tuple)):
            return self.rpc_server.monitor_command_list (self.mid, cmd)
    ...
    def screengrab (self):
        #if self.up:
        w,h = self.rpc_server.screengrab (self)
        #images = ?
        #images.is_valid?
        #get_images?
