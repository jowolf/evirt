...
class SecureJSONRPCRequestHandler(SimpleJSONRPCServer.SimpleJSONRPCRequestHandler):
    ...
    def do_POST(self):
        ...
        tok = self.headers ['X-evirt-Auth-Token']
        print tok, settings.last_tok
        print hashlib.sha224 (`settings.shared_secret` + settings.last_tok).hexdigest()

        # Use global class / obj for this?
        if tok and tok != hashlib.sha224 (`settings.shared_secret` + settings.last_tok).hexdigest():
            return self.report_4xx (403)

        settings.last_tok = tok
