...
from kvm_core import LocalVM, invalid_vm
...
def get_auth_token (seed):
    ...
    if trace:
        print `settings.shared_secret`
        print hashlib.sha224 (`settings.shared_secret`).hexdigest()
        print seed

    assert hashlib.sha224 (`settings.shared_secret`).hexdigest() == seed

    auth_seed = seed
    auth_token = hashlib.sha224 (seed + `random.random()`).hexdigest()
    settings.last_tok = auth_token
    return auth_token
...
class KvmServer (object):
    ...
    def getAuthToken (self, seed):
        return 'blah', seed

    def _verify_token_before_call (self, args):
        print 'pre_call', args

    def _gen_next_token_after_call (self, args):
        print 'post_call', args
...
if __name__ == '__main__':
  ...
  json_server.register_instance(KvmServer())
